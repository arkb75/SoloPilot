...
class ReviewerAgent:
    ...
    def _run_pytest(self, milestone_dir: Path) -> Dict[str, Any]:
        ...
        try:
            # Look for test files
            test_files = list(milestone_dir.rglob("test_*.py")) + list(milestone_dir.rglob("*_test.py"))
            ...
            try:
                with open(file_path, 'r', encoding='utf-8') as f:
                    ...
                    if lines > self.config["reviewer"]["max_file_size"]:
                        large_files.append({
                            "file": str(file_path.relative_to(milestone_dir)),
                            "lines": lines
                        })
    ...
    def _determine_status(self, static_results: Dict[str, Any], ai_review: Dict[str, Any]) -> str:
        ...
        pytest_results = static_results.get("pytest", {})
        if pytest_results.get("success") and not pytest_results.get("passed") and not pytest_results.get("no_tests"):
            return "fail"
        
        # Check AI review for high severity issues
        high_severity_count = len([
            c for c in ai_review.get("comments", [])
    ...
    def _write_review_report(self, milestone_dir: Path, review_result: Dict[str, Any]) -> None:
        ...
        report_lines.extend([
            f"---",
            f"*Generated by SoloPilot ReviewerAgent*"
        ])
