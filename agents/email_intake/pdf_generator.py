...
class ProposalPDFGenerator:
    ...
    def _get_project_description(self, requirements: Dict, understanding: Dict, conversation: Dict) -> str:
    ...
    def generate_proposal_pdf(self, conversation: Dict, timeout: int = 5) -> Tuple[Optional[bytes], Optional[str]]:
        """
        ...
        try:
            ...
            client_id = conversation.get('client_email', 'unknown-client').split('@')[0].replace('.', '-').replace('_', '-')
            ...
            logger.info(f"Generating proposal PDF for {proposal_data['clientName']} with clientId: {client_id}")
            logger.info(f"Payload keys: {list(payload.keys())}, data keys: {list(payload['data'].keys())}")
            ...
            logger.info(f"Lambda response status: {response.get('StatusCode')}, response size: {len(raw_response)} bytes")
                    ...
                    if body_json.get('success'):
                        ...
                        if pdf_base64:
                            ...
                            if s3_key:
                                ...
                                return None, f"PDF generated but not included in response. S3 key: {s3_key}"
                    else:
                        # Handle errors in the nested body
                        error_msg = body_json.get('error', body_json.get('errorMessage', 'Unknown error in body'))
            ...
            error_msg = response_payload.get('error', response_payload.get('errorMessage', 'Unknown error generating PDF'))
