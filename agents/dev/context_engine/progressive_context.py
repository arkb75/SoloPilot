...
class ProgressiveContextBuilder:
    ...
    def add_context(
        ...
        if (
            self.tier_tokens[target_tier] + estimated_tokens > tier_budget
            or self.current_tokens + estimated_tokens > self.max_tokens
        ):
    ...
    def _remove_least_relevant_from_tier(self, tier: ContextTier) -> bool:
        ...
        """
        tier_contexts = [
            (i, part) for i, part in enumerate(self.context_parts) if part["tier"] == tier.name
        ]
        ...
        least_relevant_idx, least_relevant = min(tier_contexts, key=lambda x: len(x[1]["content"]))
    ...
    def build_final_context(self, prompt: str = "", milestone_name: str = "") -> str:
        ...
        if self.skipped_contexts:
            ...
            sections.extend(["## ⚠️ Budget Management Warning", warning_msg, ""])
            self.metadata["warnings"].append(warning_msg)

        if milestone_name:
            sections.extend([f"## Milestone: {milestone_name}", ""])

        if prompt.strip():
            sections.extend(["## Current Task", prompt.strip(), ""])
        ...
        if self.metadata["escalation_reasons"]:
            sections.append(
                f"- **Escalation Reasons**: {', '.join(self.metadata['escalation_reasons'])}"
            )
        ...
        if self.skipped_contexts:
            ...
            for skip in self.skipped_contexts[:10]:  # Show max 10 for brevity
                sections.append(
                    f"| {skip['symbol']} | {skip['tier']} | {skip['tokens']} | {skip['reason']} |"
                )
    ...
    def _build_minimal_context(self, prompt: str, milestone_name: str) -> str:
        ...
        sections = ["# SoloPilot Minimal Context", "# No symbols found - using basic context", ""]
