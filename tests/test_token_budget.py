...
class TestTokenBudgetEnforcement(unittest.TestCase):
    ...
    def test_total_budget_enforcement(self):
        ...
        for i in range(5):
            ...
            success = self.builder.add_context(
                content, ContextTier.STUB, f"symbol_{i}", "stub"
            )
            if not success:
                break
    ...
    def test_smart_truncation(self):
        ...
        stub_contexts = [
            part for part in self.builder.context_parts 
            if part["tier"] == ContextTier.STUB.name
        ]
    ...
    def test_skipped_context_tracking(self):
        ...
        result = self.builder.add_context(
            large_content, ContextTier.STUB, "huge_symbol", "stub"
        )
...
class TestContextModeSelection(unittest.TestCase):
    ...
    def test_context_mode_auto_selection(self):
        ...
        for prompt in comprehensive_prompts:
            selected = engine._select_context_mode(prompt)
            self.assertEqual(selected, "COMPREHENSIVE", f"Should select COMPREHENSIVE for: {prompt}")
...
""")
    ...
    def test_balanced_mode_token_limits(self, mock_start_server):
        ...
        self.assertLessEqual(metadata["token_count"], 1500, "BALANCED mode should not exceed 1500 tokens")
    ...
    def test_minimal_mode_token_limits(self, mock_start_server):
        ...
        self.assertLessEqual(metadata["token_count"], 800, "MINIMAL mode should not exceed 800 tokens")
...
class TestCITokenValidation(unittest.TestCase):
        ...
        for mode in modes:
            with self.subTest(mode=mode):
                ...
                self.assertLessEqual(
                    ...
                    f"{mode} mode should not exceed 2000 token CI limit"
                )
    ...
    def test_performance_requirements(self):
        ...
        self.assertLess(processing_time, 1.0, "Budget enforcement should not significantly impact performance")
...
""")
    ...
    def test_balanced_mode_token_average(self, mock_start_server):
        ...
        for project_root in self.sample_projects:
            milestone_dir = project_root / "output" / "dev" / f"milestone_{self.sample_projects.index(project_root)}"
            ...
            for scenario in test_scenarios:
                ...
                self.assertLessEqual(token_count, 1500, f"Scenario '{scenario}' exceeded BALANCED limit")
        ...
        self.assertGreaterEqual(average_tokens, 800, "Average should be at least 800 tokens (not too aggressive)")
        self.assertLessEqual(average_tokens, 1500, "Average should not exceed 1500 tokens (BALANCED limit)")
