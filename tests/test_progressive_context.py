...
class TestProgressiveContextBuilder(unittest.TestCase):
    ...
    def test_context_addition_and_tracking(self):
        ...
        self.assertTrue(builder.add_context(stub_content, ContextTier.STUB, "example_function", "stub"))
        ...
        full_content = "def example_function():\n    '''Example function'''\n    return 'Hello, World!'"
        self.assertTrue(builder.add_context(full_content, ContextTier.LOCAL_BODY, "example_function", "full_body"))
    ...
    def test_final_context_building(self):
        ...
        builder.add_context("class TestClass: pass", ContextTier.LOCAL_BODY, "TestClass", "full_body")
...
class TestSymbolSelector(unittest.TestCase):
    ...
    def test_primary_target_identification(self):
        ...
        symbols = ["UserManager", "user_service", "authenticate_user", "login_handler", "create_session"]
        ...
        found_user_related = any("user" in target.lower() or "auth" in target.lower() for target in targets3)
    ...
    def test_symbol_prioritization(self):
        ...
        if auth_symbols and non_auth_symbols:
            ...
            self.assertLess(auth_index, non_auth_index, "Auth symbols should be prioritized for auth prompt")
...
"""
        ...
        milestone_data = {
            ...
            "functions": ["authenticate", "oauth_authenticate", "validate_credentials", "create_session"],
    ...
    def test_simple_task_efficiency(self):
        ...
        for prompt in simple_prompts:
            ...
            self.assertLessEqual(builder.current_tokens, 125, f"Simple task should use ≤500 tokens: {prompt}")  # 500/4
...
def authenticate(self, username, password):
    ...
    return None
"""
                builder.add_context(full_content, ContextTier.LOCAL_BODY, "authenticate", "full_body")
            
            # Should stay within limits
            self.assertLessEqual(builder.current_tokens, 1800, f"Should not exceed max tokens: {prompt}")
            self.assertLessEqual(builder.current_tokens, 375, f"Complex task should use ≤1500 tokens: {prompt}")  # 1500/4
    ...
    def test_token_efficiency_vs_quality(self):
        ...
        self.assertLessEqual(builder.current_tokens, 300, "OAuth refactor should use ≤1200 tokens")  # 1200/4
    ...
    def test_hard_token_limit_enforcement(self):
        ...
        self.assertGreater(builder.current_tokens, 1000, "Should use substantial portion of budget when hitting limit")
