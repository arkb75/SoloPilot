    ...
    def test_langchain_chroma_engine_build_context(self):
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                (milestone_path / "milestone.json").write_text(
                    '{"name": "advanced-app", "description": "Advanced application"}'
                )
                (milestone_path / "package.json").write_text(
                    '{"name": "advanced-app", "dependencies": {"express": "^4.0.0"}}'
                )
                (milestone_path / "README.md").write_text(
                    "# Advanced App\nThis is an advanced application."
                )

                context, metadata = engine.build_context(
                    milestone_path, "Create advanced API endpoints"
                )
...
"""
            ...
            with patch.dict(
                os.environ,
                {"AI_PROVIDER": "fake", "NO_NETWORK": "1", "CONTEXT_ENGINE": "lc_chroma"},
            ):
                agent = DevAgent(str(config_path))
                info = agent.context_engine.get_engine_info()
                assert info["engine"] == "legacy"
