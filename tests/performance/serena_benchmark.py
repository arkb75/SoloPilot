...
from agents.dev.context_engine import get_context_engine, LegacyContextEngine
...
'''
    ...
    def test_token_reduction_comparison(self):
        ...
        for milestone_path, prompt in self.milestones:
            ...
            with unittest.mock.patch('agents.dev.context_engine.serena_engine.subprocess.run') as mock_run:
            ...
            self.assertGreater(reduction_percentage, 0, 
                             f"Expected token reduction for {milestone_path.name}")
    ...
    def test_complex_project_handling(self):
        ...
        with unittest.mock.patch('agents.dev.context_engine.serena_engine.subprocess.run') as mock_run:
            ...
            serena_engine = SerenaContextEngine(project_root=self.temp_dir)
            serena_context, serena_meta = serena_engine.build_context(large_milestone, complex_prompt)
        serena_time = (time.time() - serena_start) * 1000
        ...
        efficiency_gain = (token_reduction / len(legacy_context) * 100) if len(legacy_context) > 0 else 0
    ...
    def _calculate_summary(cls):
        ...
        # Performance summary
        time_tests = [t for t in cls.results["tests"] if "legacy_time_ms" in t and "serena_time_ms" in t]
        if time_tests:
            ...
            cls.results["summary"]["performance"] = {
                "avg_legacy_time_ms": avg_legacy_time,
                "avg_serena_time_ms": avg_serena_time,
                "no_timeouts": all(t["legacy_time_ms"] < 30000 and t["serena_time_ms"] < 30000 for t in time_tests)
            }
