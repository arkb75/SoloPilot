...
def run_complete_pipeline(requirement_text: str, output_dir: str = None) -> dict:
    ...
    try:
        ...
        if dev_outputs:
            ...
            print(
                f"    🔍 Linting: {', '.join(linting_info['languages_supported']) or 'None available'}"
            )
        else:
        ...
        linting_overhead_pct = (
            ((dev_time - baseline_time) / baseline_time * 100) if baseline_time > 0 else 0
        )
        ...
        print("  📊 Breakdown:")
        print(
            f"    - Analysis: {analysis_time:.2f}s ({results['performance']['analysis_percentage']:.1f}%)"
        )
        print(
            f"    - Planning: {planning_time:.2f}s ({results['performance']['planning_percentage']:.1f}%)"
        )
        print(
            f"    - Development: {dev_time:.2f}s ({results['performance']['development_percentage']:.1f}%)"
        )

        if results["performance"]["linting_enabled"]:
            print(
                f"  🔍 Estimated Linting Overhead: ~{results['performance']['estimated_linting_overhead']:.1f}%"
            )
        else:
            print("  ⚠️ Real-time linting not available (missing dependencies)")
...
Examples:
  ...
  python scripts/run_solopilot.py "Build a React dashboard for sales analytics" --output custom_output
    ...
    parser.add_argument("requirement", help="Project requirement description (quoted string)")

    parser.add_argument("--output", "-o", help="Custom output directory for dev agent artifacts")

    parser.add_argument("--json", action="store_true", help="Output results as JSON")

    parser.add_argument(
        "--performance", action="store_true", help="Show detailed performance analysis"
    )
    ...
    if args.performance and results.get("success"):
        ...
        if perf.get("linting_enabled"):
            ...
            if overhead < 20:
                print("    ✅ Performance target met (<20%)")
            else:
                print("    ⚠️ Performance target missed (>20%)")
        else:
            print("  ⚠️ Real-time linting not available")
