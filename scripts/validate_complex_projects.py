...
from tests.regression.complex_projects.large_ecommerce_platform import create_large_ecommerce_project
from tests.regression.complex_projects.enterprise_saas_platform import create_enterprise_saas_project
...
class ComplexProjectValidator:
    ...
    def validate_complex_project(self, project_name: str, project_creator, target_time: int) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"  ✅ Generated {result['phases'][-1]['files_created']} files in {phase1_duration:.2f}s")
            ...
            result["phases"].append({
                ...
                "files_generated": len(manifest["milestones"]) * 3  # implementation, test, readme
            })
            print(f"  ✅ Processed {result['phases'][-1]['milestones_processed']} milestones in {phase2_duration:.2f}s")
            ...
            print(f"📊 Total Duration: {total_duration:.2f}s (Target: {target_time}s) "
    ...
    def _validate_generated_output(self, output_dir: Path, manifest: Dict[str, Any]) -> Dict[str, Any]:
                ...
                for file_path in milestone_dir.glob("implementation.*"):
                    try:
                        ...
                        if len(content) < 100:
                            ...
                            content_check["issues"].append(f"{file_path.name} lacks implementation guidance")
    ...
    def _calculate_performance_score(self, result: Dict[str, Any], target_time: int) -> float:
        ...
        validation_phase = next((p for p in result.get("phases", []) if p["name"] == "output_validation"), None)
        if validation_phase:
            failed_checks = validation_phase.get("total_checks", 0) - validation_phase.get("passed_checks", 0)
    ...
    def run_all_validations(self) -> Dict[str, Any]:
        ...
        try:
            ...
            all_results["success_count"] = sum(1 for p in all_results["projects"] if p.get("success", False))
            all_results["total_count"] = len(all_results["projects"])
            all_results["avg_performance_score"] = sum(p.get("performance_score", 0) for p in all_results["projects"]) / len(all_results["projects"])
            all_results["within_target_count"] = sum(1 for p in all_results["projects"] if p.get("within_target", False))
            ...
            print(f"  ✅ Successful Projects: {all_results['success_count']}/{all_results['total_count']}")
            print(f"  ⏱️ Within Target Time: {all_results['within_target_count']}/{all_results['total_count']}")
...
def main():
    ...
    try:
        ...
        if results["success_count"] == results["total_count"] and results["within_target_count"] >= results["total_count"] * 0.8:
            print("✅ All complex project validations passed!")
            sys.exit(0)
        else:
            print(f"⚠️ Some validations failed or exceeded target times!")
            sys.exit(1)
