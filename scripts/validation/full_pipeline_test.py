...
class FullPipelineValidator:
    ...
    def detect_bypass_attempts(self) -> List[str]:
        ...
        if ai_provider == "fake":
            bypass_attempts.append("Fake AI provider detected - real provider required for validation")
    ...
    def validate_environment(self) -> Dict[str, Any]:
        ...
        if ai_provider not in ["bedrock"]:
            validation["warnings"].append(f"AI provider '{ai_provider}' may not support all features")
        
        # Check SonarCloud configuration
        sonar_token = os.getenv("SONAR_TOKEN")
        if not sonar_token:
            validation["warnings"].append("SONAR_TOKEN not set - SonarCloud integration will be skipped")
    ...
    def phase_3_linting_validation(self, output_dir: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            self.integrity_checks["linting_executed"] = len(linting_evidence) > 0 or len(linting_configs_found) > 0
    ...
    def phase_4_sonarcloud_integration(self, project_dir: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            test_repo_url = f"https://github.com/solopilot-demo/validation-test-{self.validation_id}.git"
            
            # Attempt auto-provisioning
            auto_provision_start = time.time()
            result = client.setup_project_from_git_url(
                test_repo_url, 
                f"SoloPilot Validation Test {self.validation_id}"
            )
    ...
    def _analyze_generated_code_quality(self, files: List[Path]) -> float:
        ...
        for file_path in files:
            try:
                ...
                if any(keyword in content for keyword in ["function", "class", "def", "const", "let"]):
                    score += 1
                total_checks += 1
    ...
    def run_full_validation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            critical_phases_passed = all(
                phase["success"] for phase in self.results["phases"][:3]  # First 3 phases are critical
            )
            
            integrity_score = sum(1 for check in self.integrity_checks.values() if check) / len(self.integrity_checks)
            
            self.results["success"] = (
                critical_phases_passed and 
                not self.results["bypass_detected"] and
                integrity_score >= 0.6  # At least 60% of integrity checks passed
            )
            ...
            print(f"{success_icon} Overall Result: {'PASSED' if self.results['success'] else 'FAILED'}")
