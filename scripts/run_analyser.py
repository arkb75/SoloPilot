...
def process_requirements(input_path: str, config_path: str = None, output_dir: str = None) -> str:
    ...
    if input_path.suffix.lower() == '.zip':
        ...
        files = {
            'text': [str(input_path)] if input_path.suffix.lower() in {'.md', '.txt', '.docx'} else [],
            'images': [str(input_path)] if input_path.suffix.lower() in {'.png', '.jpg', '.jpeg', '.gif', '.bmp', '.tiff'} else [],
            'other': []
        }
...
Examples:
  ...
  %(prog)s --file brief.txt --config ./config/model_config.yaml
    ...
    input_group.add_argument(
        '--path', 
        help='Path to file, directory, or ZIP archive containing requirements'
    )
    input_group.add_argument(
        '--file',
        help='Single file to process (alias for --path)'
    )
    ...
    try:
        ...
        session_dir = process_requirements(
            input_path,
            args.config,
            args.output
        )
