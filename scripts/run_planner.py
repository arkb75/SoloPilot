...
Examples:
  ...
  %(prog)s --spec spec.json --config ./config/model_config.yaml
    ...
    input_group.add_argument("--spec", help="Path to specification.json file from analyser")
    ...
    parser.add_argument(
        "--output", help="Output directory for planning results", default="analysis/planning"
    )
    ...
    try:
        ...
        # Initialize planner
        planner = ProjectPlanner(config_path=args.config, output_dir=args.output)
