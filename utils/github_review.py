...
class GitHubReviewer:
    ...
    def post_review_to_pr(self, review_result: Dict[str, Any], pr_number: Optional[int] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            return {
                "success": False,
                "reason": "error",
                "message": f"Failed to post review: {e}"
            }
    ...
    def _post_inline_comments(self, review_result: Dict[str, Any], pr_number: int) -> Dict[str, Any]:
    ...
    def _post_single_inline_comment(self, pr_number: int, file_path: str, line_number: int, message: str) -> bool:
        """Post a single inline comment to PR."""
        try:
            # Use gh CLI to post review comment
            result = subprocess.run([
                "gh", "pr", "comment", str(pr_number),
                "--body", f"**File: {file_path}:{line_number}**\n\n{message}"
    ...
    def _post_summary_comment(self, review_result: Dict[str, Any], pr_number: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.returncode == 0:
                return {
                    "success": True,
                    "comment_length": len(summary_body)
                }
            else:
                return {
                    "success": False,
                    "error": result.stderr or "Unknown error posting summary"
                }
                
        except (subprocess.TimeoutExpired, FileNotFoundError) as e:
            return {
                "success": False,
                "error": f"Failed to post summary: {e}"
            }
