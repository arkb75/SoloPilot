...
class BaseLinter(ABC):
    ...
    def _run_command(self, cmd: List[str], cwd: Optional[Path] = None, input_text: str = "") -> Tuple[int, str, str]:
...
class ESLintLinter(BaseLinter):
    ...
    def lint_code(self, code: str, filename: str = "temp.js") -> LintResult:
        ...
        try:
            ...
            cmd = ["eslint", temp_path, "--format", "json", "--no-eslintrc", "--config", '{"extends": ["eslint:recommended"]}']
            returncode, stdout, stderr = self._run_command(cmd)
...
'''
    ...
    if "python" in manager.get_available_languages():
        ...
        if manager.has_critical_errors(results):
            ...
            print(correction_prompt[:500] + "..." if len(correction_prompt) > 500 else correction_prompt)
