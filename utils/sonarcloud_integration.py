            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        ...
                        print(
                            f"⚠️ SonarCloud API call failed (attempt {attempt + 1}/{max_retries}): {e}"
                        )
...
class SonarCloudClient:
    ...
    def create_project(
        self, project_name: str, project_key: str = None, visibility: str = "private"
        ...
        if not project_key:
            ...
            sanitized_name = re.sub(r"_+", "_", sanitized_name).strip(
                "_"
            ...
            if not re.match(r"^[a-zA-Z0-9._-]+$", project_key):
                raise ValueError(
                    f"Generated project key '{project_key}' contains invalid characters"
                )
        ...
        try:
            ...
            response = self.session.post(
                url, data=data, timeout=30  # Increased timeout for project creation
            )
            ...
            if response.status_code == 200:
                ...
                print("❌ Access denied. Check organization permissions and token scopes.")
                print(f"   Organization: {self.organization}")
                print("   Required permissions: Project Creation")
                return None
            elif response.status_code == 401:
                print("❌ Authentication failed. Check SONAR_TOKEN validity.")
                return None
            else:
    ...
    def setup_project_from_git_url(
        self, git_url: str, project_name: str = None
    ) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            project_result = self.create_project(
                project_name=project_name, project_key=parsed["project_key"]
            )

            if project_result:
                project_result.update(
                    {"git_url": git_url, "git_owner": parsed["owner"], "git_repo": parsed["repo"]}
                )

                print("🎉 SonarCloud project setup complete!")
    ...
    def _configure_main_branch(self, project_key: str, branch_name: str):
        ...
        try:
            response = self.session.post(
                ...
                data={"project": project_key, "name": branch_name},
                timeout=10,
            )
    ...
    def generate_review_summary(self) -> Dict[str, Any]:
        ...
        """
        if not self.is_available():
            return {"available": False, "reason": "offline_mode" if self.no_network else "no_token"}
    ...
    def _analyze_quality_data(
        self, metrics: Optional[Dict], issues: List[Dict], quality_gate: Optional[Dict]
    ) -> Dict[str, Any]:
        ...
        if vulnerabilities > 0:
            analysis["recommendations"].append(
                f"Fix {vulnerabilities} security vulnerabilities immediately"
            )
        if bugs > 0:
            analysis["recommendations"].append(f"Address {bugs} bugs before promotion")
        if coverage < 70:
            analysis["recommendations"].append(
                f"Improve test coverage from {coverage:.1f}% to >70%"
            )
        if code_smells > 15:
            analysis["recommendations"].append(
                f"Refactor {code_smells} code smells for maintainability"
            )
        ...
        for issue in critical_issues:
            if issue.get("type") == "VULNERABILITY":
                analysis["blockers"].append(
                    f"Security vulnerability: {issue.get('message', 'Unknown')}"
                )
...
def main():
    ...
    for url in test_urls:
        ...
        if parsed:
            print(f"  ✅ {url}")
            print(
                f"     → Owner: {parsed['owner']}, Repo: {parsed['repo']}, Project Key: {parsed['project_key']}"
            )
        else:
    ...
    if client.auto_detect_project():
        ...
        print("  ⚠️ Could not auto-detect project")
