...
class Context7Bridge:
    ...
    def _check_context7_available(self) -> bool:
        ...
        try:
            result = subprocess.run(
                ["context7", "--version"], capture_output=True, text=True, timeout=10
            )
    ...
    def scout_milestone_pitfalls(
        self, milestone_name: str, description: str, tech_stack: List[str]
    ) -> Optional[str]:
        """
    ...
    def scout_implementation_patterns(
        self, milestone_name: str, tech_stack: List[str]
    ) -> Optional[str]:
        """
...
Focus on practical implementation issues, performance problems, security concerns, and architectural mistakes developers commonly make."""
    ...
    def generate_milestone_insights(
        self, milestone: Dict[str, Any], tech_stack: List[str]
        ...
        """
        if not self.enabled:
            return {"pitfalls": None, "patterns": None, "testing": None, "enabled": False}
    ...
    def format_insights_for_readme(self, insights: Dict[str, Optional[str]]) -> str:
        ...
        if insights.get("pitfalls"):
            sections.append(
                f"""## ⚠️ Common Pitfalls (Context7 Insights)
...
{insights["pitfalls"]}"""
        ...
        if insights.get("patterns"):
            sections.append(
                f"""## 🏗️ Implementation Patterns (Context7 Insights)
...
{insights["patterns"]}"""
        ...
        if insights.get("testing"):
            sections.append(
                f"""## 🧪 Testing Strategies (Context7 Insights)
